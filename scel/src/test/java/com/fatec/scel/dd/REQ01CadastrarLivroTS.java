		...
		while (!ManipulaExcel.getCellData(linha, 0).equals("final".trim())) {
			...
			try {
				...
				if (ManipulaExcel.getCellData(linha, 3).equals("Lista de livros")) {
					logger.info(">>>>>> 3. Processando a linha = " + linha + "- ISBN = " + ManipulaExcel.getCellData(linha, 0)
					+ "- RE = " + ManipulaExcel.getCellData(linha, 3));
					assertEquals(ManipulaExcel.getCellData(linha, 3),pageCadastrarLivro.getResultadoCadastroComSucesso());
				}
				driver.manage().timeouts().implicitlyWait(0, TimeUnit.SECONDS);// desliga a espera implicita	
